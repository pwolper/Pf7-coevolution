...
* Tasks :noexport:
plot sfs and theta/recomb for two populations. Which two? Set up pipeline for this.
** DONE filter the vcf file for snps with higher quality (VQSLOD > 5.0)
- rerun some of the faststructure with this smaller dataset. are the results the same? Also maybe rerun the DAPC?
** DONE SFS plotting. plot the sfs with neutral model expectation and zoomed windows.

** TODO Recombination rate in genes and intergenic regions
- following the 4-gamete rule, how is the recombination rate in genes? Compare with genetic diversity stats in genes.
- Non-gene regions RM and maybe filter out regions with less than 30 SNPs?

** TODO Population structure analysis
- fastSTRUCTURE (https://github.com/rajanil/fastStructure)
  https://www.biorxiv.org/content/10.1101/181610v1.full.pdf
- DAPC and PCA plot
- redo the dapc according to the tutorial writen by the author.
- How do I interpret the likelihood values for choosing K in faststructure? delta K convergence plots?
